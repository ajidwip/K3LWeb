<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/Site.Master" CodeBehind="masterpertanyaanmasukgudang.aspx.cs" Inherits="k3lkendaraanweb.Master.masterpertanyaanmasukgudang" %>
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    <%:Styles.Render("~/Content/plugins/dataTables/dataTablesStyles") %>
    <%:Scripts.Render("~/plugins/dataTables")%>
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-lg-10">
            <h2>Pertanyaan Kondisi Kendaraan Masuk Gudang</h2>
            <ol class="breadcrumb">
                <li>
                ...
                <li class="active">
                    <strong>Pertanyaan Kondisi Kendaraan Masuk Gudang</strong>
                </li>
            </ol>
        </div>
        <div class="col-lg-2">
                                        ...
                                        <tbody>
                                            <tr id="{{$index}}" ng-repeat="x in master|filter:activeId1" class="gradeX">
                                                ...
                                                <td>
                                                  ...
                                                  <label class="hidewhenclick_{{$index}}">{{x.Pernyataan}}</label>
                                                    <input type="text" id="Pernyataan_{{$index}}" class="control_{{$index}}" style="display: none" ng-value="x.Pernyataan" />
                                                </td>
                                                <td>
                                        ...
                                        <table id="tablemenudetail" datatable="ng" class="table table-striped table-bordered table-hover dataTables-example">
                                            ...
                                            <tbody>
                                                <tr id="{{$index}}" ng-repeat="x in masterdtl|filter:activeId" class="gradeX">
                                                    ...
                                                    <td ng-click="inlineEditdtl1($index)">
                                                        <label class="hidewhenclickdtl_{{$index}}">{{x.PernyataanDetail}}</label>
                                                        <input type="text" id="PernyataanDetail_{{$index}}" class="controldtl_{{$index}}" style="display: none" ng-value="x.PernyataanDetail" />
                                                    </td>
    ...
    <script>
        ...
        window.ukuran = [];
        window.ukurandtl = [];
        window.id1Dtl = ['NoHeader', 'NoDetail', 'PernyataanDetail'];
        window.url1 = "<%=ResolveUrl("~/Master/masterpertanyaanmasukgudang.aspx/GetData") %>";
        window.urldtl1 = "<%=ResolveUrl("~/Master/masterpertanyaanmasukgudang.aspx/GetDatadtl") %>";
        window.url2 = "<%=ResolveUrl("~/Master/masterpertanyaanmasukgudang.aspx/crud1") %>";
        window.Key = 'No';
        window.url2dtl = "<%=ResolveUrl("~/Master/masterpertanyaanmasukgudang.aspx/crudDtl") %>"
        window.filterdtl = 'No';
        window.filter1 = 'No';
